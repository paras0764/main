...
function pdfHeaderLabel(key) {
  ...
  // Keep it readable but prevent random line breaks
...
export default function PartialRgp({
  ...
  async function exportExcel() {
    try {
      ...
      XLSX.utils.book_append_sheet(wb, ws, "Partial RGP");

      const filename = `Partial_RGP_${formatTodayStamp()}.xlsx`;
      ...
      downloadBlob(
        ...
        `Partial_RGP_${formatTodayStamp()}.csv`
      );
  ...
  async function exportPDF() {
    try {
      ...
      doc.setProperties({
        title: "Partial RGP Report",
        subject: "Partial RGP (Active/Partial) export",
        keywords: "RGP, report, export, PDF",
        creator: "Partial RGP Dashboard",
      });
      ...
      const drawPageChrome = (pageNumber, totalPages) => {
        ...
        doc.text("Partial RGP", margin + inner, margin + 22 + inner / 2);
        ...
        doc.text("Generated by Partial RGP Dashboard", margin, footerY);
      };
      ...
      // Final save
      const filename = `Partial_RGP_${formatTodayStamp()}.pdf`;
      doc.save(filename);
    } catch (err) {
      console.error(err);
  ...
  return (
    <div className="rgp-shell">
      ...
      <div className="rgp-header">
        <div>
          <h2 className="rgp-title">📋 Partial RGP Dashboard</h2>
          <div className="chips">
            <span className="chip">🎯 Showing <b>Partial RGP</b> Only</span>
            <span className="chip">📊 {closedRows.length} Partial Records</span>
