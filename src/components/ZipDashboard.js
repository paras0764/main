...
// Fetch Zip Quality Data (rates)
const fetchZipQualityData = async () => {
  try {
    ...
    const zipTypeIndex = headers.findIndex(h => h.includes('zip') || h.includes('type'));
    ...
    if (zipTypeIndex === -1 || colorIndex === -1 || priceIndex === -1) return [];

    const zipData = [];
    for (let i = 1; i < data.values.length; i++) {
      const r = data.values[i] || [];
      const type = (r[zipTypeIndex] || '').toString().trim();
      const color = (r[colorIndex] || '').toString().trim();
      const price = parseFloat((r[priceIndex] || '0').toString().replace(/[^\d.]/g, '')) || 0;
      if (type && color) {
        zipData.push({ type, color, price });
      }
    }
    return zipData;
  } catch (err) {
    console.warn('Could not fetch Zip Quality Data:', err);
    return [];
  }
};

// Generate Full Zip Purchase Order PDF Document
export const generateZipPoPdf = async (row, zipQualityData = []) => {
  ...
  const getZipPrice = (zType, zColor) => {
    if (!zType || !zColor || !zipQualityData?.length) return 0;
    const normT = zType.toString().trim().toLowerCase();
    const normC = zColor.toString().trim().toLowerCase();
    const item = zipQualityData.find(it => (it.type || '').trim().toLowerCase() === normT && (it.color || '').trim().toLowerCase() === normC);
  ...
  placements.forEach(placement => {
    ...
    const zipType = placementZipTypes[placement] || 'Standard Zip';
    colors.forEach(color => {
      const qty = colorBreakdownObj[color] || totalPieces || 0;
      const zipColor = zipSelections[color] || color;
      ...
      if (!zipTypeSummary[zipType]) zipTypeSummary[zipType] = 0;
      zipTypeSummary[zipType] += reqQty;
  ...
  const totalQuantityAll = zipBody.reduce((sum, r) => sum + (parseInt(r[4]) || 0), 0);
  const totalCostVal = parseFloat(row['Total Cost (₹)']) || totalZipCost;

  const zipFoot = [
    ['', '', '', '', `T QTY: ${totalQuantityAll || totalPieces}`, 'T COST:', totalCostVal > 0 ? totalCostVal.toFixed(2) : '-']
  ];
  ...
  const summaryData = Object.entries(zipTypeSummary).map(([zipType, totalQty]) => ({ zipType, totalQty }));
...
const ZipPurchaseDashboard = () => {
  ...
  const [zipQualityRates, setZipQualityRates] = useState([]);
  ...
  const fetchData = async () => {
    try {
      ...
      const [sheetRes, zipRates] = await Promise.all([
  ...
  // Direct PO PDF Download Action
  const handleDownloadPo = async (row) => {
    if (!row || downloadingPoId) return;
    setDownloadingPoId(row.id);
    try {
      const { doc, filename } = await generateZipPoPdf(row, zipQualityRates);
      doc.save(filename);
      setFeedbackMessage(`✅ Successfully downloaded PO for Lot ${row['Lot Number'] || ''}`);
  ...
  const downloadPDF = () => {
    ...
    doc.save(`zip-orders-summary-report-${new Date().toISOString().slice(0, 10)}.pdf`);
  };
  ...
  if (loading) {
    return (
      <div style={styles.loadingContainer}>
        ...
        <p style={styles.loadingText}>Loading Zip Purchase Orders...</p>
      </div>
    );
  }
      ...
      <div style={styles.header}>
        <div style={styles.headerLeft}>
          ...
          <div style={styles.headerContent}>
            ...
            <p style={styles.subtitle}>Manage, track and re-download all zip material purchase orders</p>
          </div>
        </div>
        <div style={styles.headerActions}>
          <div style={styles.downloadButtons}>
            <button 
              ...
              title="Search lot to re-download PO"
            >
              📥 Re-download PO
            </button>
                      ...
                      <td style={styles.actionCell}>
                        <div style={styles.actionBtnGroup}>
                          <button
                            ...
                            title="Re-download Purchase Order PDF"
                          >
        ...
        <div style={styles.modalOverlay} onClick={() => setShowLotSearchModal(false)}>
          <div style={styles.modalContent} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <h2 style={styles.modalTitle}>🔍 Quick Search & Re-download Zip PO</h2>
                ...
                {matchingLotOrders.length === 0 ? (
                  ...
                  matchingLotOrders.map(ord => (
                    <div key={ord.id} style={styles.lotResultCard}>
                      <div>
                        <strong style={{ fontSize: '15px', color: '#1e3a8a' }}>Lot #{ord['Lot Number']}</strong>
      ...
      {previewPoData && (
        <div style={styles.modalOverlay} onClick={() => setPreviewPoData(null)}>
          <div style={{...styles.modalContent, maxWidth: '800px'}} onClick={e => e.stopPropagation()}>
            <div style={styles.modalHeader}>
              <h2 style={styles.modalTitle}>📄 Purchase Order Preview: Lot #{previewPoData['Lot Number']}</h2>
            ...
            <div style={styles.modalBody}>
              ...
              <div style={{ marginTop: '20px' }}>
                <h4 style={{ color: '#1e3a8a', marginBottom: '8px' }}>Zip Placements & Specifications:</h4>
              ...
              <div style={{ marginTop: '20px' }}>
                <h4 style={{ color: '#1e3a8a', marginBottom: '8px' }}>Color Breakdown:</h4>
...
export default ZipPurchaseDashboard;